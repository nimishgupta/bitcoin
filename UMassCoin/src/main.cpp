...
#define NCOINS_PER_BLOCK_MINED 50

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = NCOINS_PER_BLOCK_MINED * COIN;
...
static const int64 EXPECTED_BLOCK_MINING_TIME_MIN = 1;
static const int64 READJUST_MINING_DIFFICULTY_AFTER_MIN = 15;

#define MIN_TO_SEC(m) ((m) * 60)

static const int64 nTargetTimespan = MIN_TO_SEC (READJUST_MINING_DIFFICULTY_AFTER_MIN);
static const int64 nTargetSpacing = MIN_TO_SEC (EXPECTED_BLOCK_MINING_TIME_MIN);
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The day Arpit thought Umasscoin should start";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
